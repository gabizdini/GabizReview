...
function hexToHsva(hex: string): { h: number; s: number; v: number } {
...
function hsvaToHex(h: number, s: number, v: number): string {
  const sNorm = s / 100;
  const vNorm = v / 100;
  const c = vNorm * sNorm;
...
function ColorPickerContent({
  ...
  const initial = hexToHsva(initialColor);
  ...
  const currentHex = hsvaToHex(hue, sat, bright);
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1">
            <span className="text-base font-mono font-medium text-neutral-700 dark:text-neutral-300">
              #
            </span>
            <input
              type="text"
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {QUICK_COLORS.map((color) => (
            <button
              ...
              className={`group relative h-8 w-full rounded-lg transition-all duration-200 hover:scale-110 hover:shadow-md ${
              ...
              title={color.name}
            >
              <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-white opacity-0 drop-shadow transition-opacity group-hover:opacity-100">
                {color.name.charAt(0)}
              </span>
            </button>
          ))}
...
export function ColorPickerModal({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full max-w-sm rounded-2xl bg-white p-4 shadow-2xl transition-all dark:bg-neutral-900"
        role="dialog"
